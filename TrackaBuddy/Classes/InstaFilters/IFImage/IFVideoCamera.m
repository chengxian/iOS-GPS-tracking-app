...
//    MingleloopAppDelegate *appdelegate = (MingleloopAppDelegate*)[UIApplication sharedApplication].delegate;
    ...
    [UserDataSingleton sharedSingleton].MediaData=UIImageJPEGRepresentation(image, 1.0);
...
{
    CGFloat imageWidth = self.rawImage.size.width;
    CGFloat imageHeight = self.rawImage.size.height;
    
    CGFloat frameWidth = self.gpuImageView_HD.frame.size.width;
    CGFloat frameHeight = self.gpuImageView_HD.frame.size.height;
    
    CGFloat widthRate = imageWidth / frameWidth;
    CGFloat heightRate = imageHeight / frameHeight;
    CGFloat rate = widthRate > heightRate ? widthRate : heightRate;
    
    gpuImageView_HD.frame = CGRectMake((frameWidth - imageWidth / rate) / 2, (frameHeight - imageHeight / rate) / 2, self.rawImage.size.width / rate, self.rawImage.size.height / rate);
    
//    CGRect ffff = gpuImageView.frame;
//    ffff.size.width = self.rawImage.size.width / rate;
//    ffff.size.height = self.rawImage.size.height / rate;
//    gpuImageView.frame = ffff;
...
//        }
        NSInteger width = [UserDataSingleton sharedSingleton].screenWidth - 20;
        gpuImageView = [[GPUImageView alloc] initWithFrame:CGRectMake(10, 75, width, width)];
        [gpuImageView setContentMode:UIViewContentModeScaleAspectFit];
        [gpuImageView_HD setContentMode:UIViewContentModeScaleAspectFit];
    }
    ...
    [gpuImageView_HD setContentMode:UIViewContentModeScaleAspectFit];
//    gpuImageView_HD.frame = CGRectMake(0, 0, 200, 200);
